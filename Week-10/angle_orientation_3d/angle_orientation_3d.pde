...
void draw() {
  ...
  if (userList.size() > 0) {
    ...
    if ( kinect.isTrackingSkeleton(userId)) {
      ...
      // GET THE ANGLE CREATE BETWEEN THESE TWO ON THE X & Z AXES
      float angle = atan2(rShoulder.x - lShoulder.x, rShoulder.z - lShoulder.z);
      ...
      // MIXING 2D WITH 3D? YES PLEASE!
      hint(DISABLE_DEPTH_TEST);
      camera(); // FIX CAMERA INVERSION
      fill(0);
      textSize(50);
      text(abs(degrees(angle)), 60, 60);
      noFill();
