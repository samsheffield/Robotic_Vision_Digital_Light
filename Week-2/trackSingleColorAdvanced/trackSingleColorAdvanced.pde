...
// use b and -,+ to adjust blur
// use space to display non-blurred video while tracking from blurred image
...
PImage mirrorNoBlur; // mirrored image without blur
...
void setup() {
  ...
  mirrorNoBlur = createImage(width, height, RGB);
...
void draw() {
  ...
  cam.loadPixels();
  mirror.loadPixels();
  mirrorNoBlur.loadPixels();
  for (int x = 0; x < width; x++) {
    for (int y = 0; y < height; y++) {
      int currentPixel = (cam.width-x-1) + (y * cam.width);
      mirror.set(x,y,cam.pixels[currentPixel]);

      if(!showBlur) mirrorNoBlur.set(x,y,cam.pixels[currentPixel]); // backup mirrored image
    }
  }
  mirror.updatePixels();
  mirrorNoBlur.updatePixels();
  ...
  // are we showing blurred image or not? display proper image
  if(!showBlur){
    image(mirrorNoBlur, 0, 0);
  }else{
    image(mirror, 0, 0);
  }
